...
// FIX: Robust playerStart to ensure 'Now Playing' message is sent
  ...
  try {
    ...
    if (channel) {
      ...
      try {
        ...
        // If message sending fails, just return and do not store the message.
        return; 
      }
...
client.on('interactionCreate', async interaction => {
  ...
  if (['play', 'skip', 'stop', 'queue', 'nowplaying', 'pause', 'resume', 'shuffle', 'loop', 'volume', '247'].includes(commandName) && !voiceChannel) {
    return interaction.reply({ content: `${config.emojis.error} You must be in a voice channel to use this command.`, flags: 64 }); // Use flags: 64 for ephemeral
  }
